...
--UPDATE TO_STEP of FLOW FL_CT_110
UPDATE SIAT_CT.FLOW f SET f.TO_STEP = (SELECT st.ID FROM SIAT_CT.STEP st WHERE st.CODE = 'ST_CT_55') WHERE f.CODE='FL_CT_110';
/*ADD FIELD_GROUP for <<Modification Request Reason>>*/
INSERT INTO SIAT_CT.FIELD_GROUP (ID,LABEL_EN,LABEL_FR) VALUES (SIAT_CT.FIELD_GROUP_SEQ.NEXTVAL,'Modification Request Reason','Raison Demande de Modification');

UPDATE SIAT_CT.FILE_FIELD ff SET ff.GROUP_ID = (SELECT fg.ID FROM SIAT_CT.FIELD_GROUP fg WHERE fg.LABEL_EN = 'Modification Request Reason' AND fg.LABEL_FR ='Raison Demande de Modification') WHERE ff.CODE = 'INFORMATIONS_GENERALES_OBSERVATIONS';