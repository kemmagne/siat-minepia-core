...
/**
 * The Class FileFieldValueVtMINEPDED.
 */
public class FileFieldValueVtMINEPDED
{/**
	 * Generate file field value_ v t_ minepded.
	 *
	 * @param file
	 *           the file
	 * @param fileField
	 ...
	 */
	public static FileFieldValue generateFileFieldValueVtMINEPDED(final File file, final FileField fileField,
			final org.guce.siat.jaxb.ap.VT_MINEPDED.DOCUMENT document,
			final ApplicationPropretiesService applicationPropretiesService)
	{
		...
		switch (fileField.getCode())
		{
			...
			case "COMMENTAIRE":
			{
				if (document.getCONTENT() != null)
				{
					fileFieldValue.setValue(document.getCONTENT().getCOMMENTAIRE());
				}
				break;
			}
			default:
